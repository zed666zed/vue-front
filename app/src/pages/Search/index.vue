...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getData()
      // this.$store.dispatch('getSearchList',{}) 请求只发一次
    },
    computed:{
    ...
    methods:{
      ...
      removeCategoryName(){
        ...
        this.getData()
        if(this.$route.params) //进行路由跳转是因为网址不匹配
          this.$router.push({
            name:'search',
      ...
      let newOrder=""
        if(flag==oldFlag)
        {
          newOrder=`${flag}:${oldSort=='desc'?'asc':'desc'}`
        }else{
          newOrder=`${flag}:desc}`
        }
    ...
    watch:{
      $route(newValue,oldValue){  //当前search路由内容的变化
        Object.assign(this.searchParams,this.$route.query,this.$route.params)
        this.getData()
