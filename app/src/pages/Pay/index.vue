...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async open() {
        let url=await QRCode.toDataURL(this.payInfo.codeUrl)//返回的是promise 得用await接收
        this.$alert(`<img src=${url} />`, '请支付', {
          dangerouslyUseHTMLString: true,
          center:true,
          showCancelButton:true,
          cancelButtonText:'支付出现问题',
          confirmButtonClass:'支付成功',
          showClose:false,
          roundButton:true,
           beforeClose:(type,instance,done)=>{//关闭弹窗的配置 得用箭头
           if(type=='cancel'){
           alert('联系管理员')
