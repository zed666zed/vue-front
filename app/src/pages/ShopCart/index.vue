      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cart.skuNum" minnum="1" class="itxt" @change="handle('change',$event.target.value,cart)">
    ...
    methods:{
      ...
      async updateChecked(cart,event){
        try {
          ...
          //需要让promise显型 其他不用让它显型是因为数据已经返回到store中 接口返回成功与失败一般要这样操作,返回数据就不用
