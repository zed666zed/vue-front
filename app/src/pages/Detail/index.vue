  ...
  export default {
    ...
    methods:{
      changeActive(saleAttrValue,arr){
        ...
        saleAttrValue.isChecked=1 //内层赋值
      },
      changeSkuNum(event){
        let value=event.target.value*1 //秒 排除非法输入
      ...
      async addShopcar(){
        try {
          ...
          // reqAddOrUpdateShopCart(this.$route.params.skuid,this.skuNum)
          sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))//会覆盖掉key为SKUINFO的数据
          this.$router.push({
            name:'addcartsuccess',
